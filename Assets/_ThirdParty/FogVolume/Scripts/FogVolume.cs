...
/*
    ...
    * Increased quality of depth texture. Removes issues in large scenes with high camera far clip.
      ...
      ----[ v 3.4.4 ] ----
    * Updated to 2021.2.14f1
    * 
    *  ----[ v 3.4.5 ] ----
    * Updated to 2022.2.2f1
    * Added functionality to save temporal material to disk.
    * Improved compilation times by replacing multi_compile with shader_feature. That's why material must be saved.
    * Created custom button styles for the inspector
    * Added an option to make the gradient affect the ambient color
    * Fixed the Shader warning in 'Hidden/Upsample': use of potentially uninitialized variable (BilateralUpsample) at line 167 (on d3d11)
    * Fixed errors when creating debug miniature in FogVolumeDirectionalLight.cs
*/
...
public class FogVolume : MonoBehaviour
{
    ...
    public bool GradientAffectAmbient = false;
    ...
    void CreateMaterial2()
    {
        ...
        if (SaveMaterials && RequestSavingMaterials)
        {
            ...
            if (path != "")
                AssetDatabase.CreateAsset(fogMaterial, path);

        }
        else
        {
            CreateUnsavedMaterial();
        }
#else
			CreateUnsavedMaterial();
#endif
    ...
    void CreateUnsavedMaterial()
    {
        FogVolumeShader = Shader.Find("Hidden/FogVolume");
        fogMaterial = new Material(FogVolumeShader);
#if UNITY_EDITOR
        fogMaterial.name = m_prevFVName + " (Not saved)";
#endif
    ...
    void OnEnable()
    {
        //Debug.Log("Luz: " + Sun.name);
        //Debug.Log("Dirección: " + -Sun.transform.forward);
    ...
    */
    void OnDisable()
    {
        ...
        {
            fogMaterial = null;
            FogRenderer.sharedMaterial = null;
        }
    }
    ...
    void ToggleKeyword()
    {
        if (IsVisible)
        {
            ...
            if (FogMaterial)
            {
                ...
                if (GradientAffectAmbient && Gradient != null)
                    FogMaterial.EnableKeyword("_FOG_GRADIENT_AFFECT_AMBIENT");
                else
                    FogMaterial.DisableKeyword("_FOG_GRADIENT_AFFECT_AMBIENT");

                // Debug.Log("_FOG_GRADIENT " + FogMaterial.IsKeywordEnabled("_FOG_GRADIENT"));
