...
namespace BoingKit
{
    ...
    public class BoingBones : BoingReactor
    {
        ...
        public LayerMask targetLayer; // Layer để tìm kiếm GameObjects
        ...
        private void Awake()
        {
            // Tự động tìm kiếm và khởi tạo các collider trong scene
            InitializeColliders();
        }

        private void InitializeColliders()
        {
            // Tìm tất cả GameObjects trong scene thuộc layer được chỉ định
            GameObject[] objectsInLayer = FindObjectsByType<GameObject>(FindObjectsSortMode.None)
                .Where(go => targetLayer == (targetLayer | (1 << go.layer)))
                .ToArray();

            // Lấy tất cả Collider từ các GameObject này
            UnityColliders = objectsInLayer
                .SelectMany(go => go.GetComponents<Collider>())
                .Where(c => c != null && c.enabled && !c.isTrigger) // Lọc collider hợp lệ
                .ToArray();
        }
