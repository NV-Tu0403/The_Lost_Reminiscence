...
public class FogVolumeEditor : Editor
{
    ...
    {
        GUIStyle customButtonStyle = new GUIStyle(skin.button);
        ...
        {
            // GUILayoutOption[] iconLayoutOptions = new GUILayoutOption[] { GUILayout.Width(16), GUILayout.Height(16) };
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(GUIContent.none, new GUIContent(DiscIcon), GUILayout.Width(16), GUILayout.Height(16));

            _target.SaveMaterials = EditorGUILayout.Toggle(new GUIContent("Save Material to disk", "Save material before building to ensure that the current shader variants are compiled"), _target.SaveMaterials);
            GUILayout.EndHorizontal();

            GUILayout.Space(5);
            if (!_target.SaveMaterials)
            {
                //GUILayout.Space(10);
                EditorGUILayout.HelpBox("Using temporal material. Save to disk before buiding", MessageType.Warning);
        ...
        if (GUILayout.Button("Lighting", customButtonStyle))
        ...
        if (_target._FogType == FogVolume.FogType.Textured)
            if (GUILayout.Button(PointLightTittle, customButtonStyle))
        ...
        //GUILayout.EndHorizontal();
        if (GUILayout.Button(new GUIContent("Color management", ShowInspectorTooltips ? "Color postprocessing such as contrast, brightness and tonemapper" : ""), customButtonStyle))
        ...
        if (/*_target.ColorAdjust && */_target.showColorTab)
        {
            ...
            if (_target.Tonemap && _target.ColorAdjust)
                _target.Exposure = EditorGUILayout.Slider("Exposure", _target.Exposure, 1f, 5);
        }
        #endregion
        ...
        if (_target._FogType == FogVolume.FogType.Textured)
        {
            ...
            if (GUILayout.Button("Renderer", customButtonStyle))
            ...
            if (_target.showVolumeProperties)
            {
                ...
                _target._DebugMode = (FogVolume.DebugMode)EditorGUILayout.EnumPopup(VariableField("View mode: ", ""), _target._DebugMode);
        ...
        #region Noise
        if (GUILayout.Button("Noise", customButtonStyle))
        ...
        #region Gradient
        if (GUILayout.Button("Gradient", customButtonStyle))
        ...
        if (_target.showGradient)
        {
            ...
            if (_target.EnableGradient)
            {
                ...
                _target.GradientAffectAmbient = EditorGUILayout.Toggle("Affect Ambient", _target.GradientAffectAmbient);
                GUILayout.Space(10);
        ...
        EditorGUI.indentLevel++;
        if (GUILayout.Button("Other", customButtonStyle))
        ...
        #region Info
        if (GUILayout.Button("Info", customButtonStyle))
        ...
        if (_target.generalInfo)
        {
            ...
            EditorGUILayout.LabelField("Version 3.4.5");
            EditorGUILayout.LabelField("Release date: July 2023");
            ...
            if (_target.GameCameraGO)
            {
                ...
                if (GUILayout.Button("Select " + _target.GameCameraGO.name, customButtonStyle))
                    Selection.activeGameObject = _target.GameCameraGO;
            }
            else
                GUILayout.Button("No valid camera found", customButtonStyle);
            GUILayout.EndVertical();
            GUILayout.Space(10);
            #endregion
