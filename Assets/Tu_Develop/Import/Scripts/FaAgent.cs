...
namespace Tu_Develop.Import.Scripts
{
    public class FaAgent : MonoBehaviour, FaInterface
    {
        ...
        private EventReference stateInstance;

        private EventInstance _stateInstance;

        // Biến để theo dõi trạng thái, tránh gọi FMOD mỗi frame
        private FaState _previousState;
        
        private Transform? _targetPositionHelper;

        private readonly Dictionary<string, float> _cooldownTimers = new();

        [SerializeField] private FaState state = FaState.Idle;
        ...
        private void SetUpFMODAudio()
        {
            // Khởi tạo FMOD
            if (!stateInstance.IsNull)
            {
                _stateInstance = RuntimeManager.CreateInstance(stateInstance);
                RuntimeManager.AttachInstanceToGameObject(_stateInstance, transform);
                _stateInstance.start();
            }
        ...
        private void InitializePlayerConfig()
        {
            ...
            // thêm máu của player vào Blackboard
        ...
        private void InitializeFaConfig()
        {
            if (faBha != null && faBha.BlackboardReference.GetVariableValue("FaState", out FaState currentState))
                state = currentState;
        }

        private void OnDestroy()
        {
            _stateInstance.stop(STOP_MODE.IMMEDIATE);
            _stateInstance.release();
        }
        ...
        public void OnPlayerCommand(string command)
        {
            ...
            {
                // Lệnh: move x y z
                if (float.TryParse(parts[1], out var x) && float.TryParse(parts[2], out var y) &&
        ...
        private void Update()
        {
            ...
            foreach (var key in keys) _cooldownTimers[key] = Mathf.Max(0, _cooldownTimers[key] - Time.deltaTime);

            UpdateCooldownToCanvas();
            if (faBha == null) return;
            faBha.BlackboardReference.GetVariableValue("FaState", out state);
            UpdateSoundByState();
        ...
        {
            // Chỉ gửi lệnh đến FMOD khi state thực sự thay đổi
            if (state != _previousState)
            {
                // "State" là tên Parameter bạn đặt trong FMOD
                _stateInstance.setParameterByName("State", (float)state);
