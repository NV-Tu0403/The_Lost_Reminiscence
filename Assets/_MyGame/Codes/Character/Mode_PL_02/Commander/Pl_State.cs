    ...
    public override void HandleAction(CharacterActionType type)
    {
        switch (type)
        {
            ...
            case CharacterActionType.Attack:
                ...
                _stateMachine.SetState(new AttackState(_stateMachine, _playerEvent));
                break;
            case CharacterActionType.ThrowWeapon:
                // Thực hiện hành động ném vũ khí
                _stateMachine.SetState(new ThrowWeaponState(_stateMachine, _playerEvent));
                break;
        }
    }
}
...
public class ThrowWeaponState : StatePlayer
{
    public ThrowWeaponState(CharStateMachine stateMachine, PlayerEvent playerEvent) : base(stateMachine, playerEvent, CharacterStateType.ThrowWeapon)
    {
    }
    /// <summary>
    /// Xử lý hành động từ giao diện người dùng.
...
public class ThrowItemState : StatePlayer
{
    public ThrowItemState(CharStateMachine stateMachine, PlayerEvent playerEvent) : base(stateMachine, playerEvent, CharacterStateType.ThrowItem)
    {
    }
    /// <summary>
    /// Xử lý hành động từ giao diện người dùng.
